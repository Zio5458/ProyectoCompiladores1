%{
  ...
  void yyerror(void*, const char* s){ fprintf(stderr, "Parse error: %s\n", s); }
%}
...
%token TOK_TYPE TOK_ID TOK_NUM TOK_BOOL
...
%%
program : /* empty */ { $$ = new Program(); g_program = $$; }
        | program function { $1->add($2); $$ = $1; }
        ;

function : TOK_FN TOK_ID LPAREN param_list RPAREN opt_ret block
         { auto f = new Func($2); for(auto p:*$4) f->addParam(p); delete $4; f->setRet($6?string($6):string()); free($6); f->setBody($7); $$=f; }
         ;

opt_ret : /* empty */ { $$ = (char*)nullptr; }
        | ARROW TOK_TYPE { $$ = $2; }
        ;

param_list : /* empty */ { $$ = new vector<Param*>(); }
           | nonempty_params { $$ = $1; }
           ;

nonempty_params : param { $$ = new vector<Param*>(); $$->push_back($1); }
                | nonempty_params COMMA param { $1->push_back($3); $$ = $1; }
                ;

param : TOK_ID ':' TOK_TYPE { $$ = new Param($1,$3); }
      ;

block : LBRACE stmts RBRACE { $$ = $2; }
      ;

stmts : /* empty */ { $$ = new Block(); }
      | stmts stmt { $1->add($2); $$ = $1; }
      ;

stmt : TOK_LET TOK_ID '=' expr SEMI { $$ = new LetStmt($2, "infer", $4); }
...
expr   : expr OR term   { $$ = new Binary($1, "||", $3); }
...
term   : term PLUS factor { $$ = new Binary($1, "+", $3); }
       | term MINUS factor{ $$ = new Binary($1, "-", $3); }
       | factor          { $$ = $1; }
       ;

factor : factor MUL primary { $$ = new Binary($1, "*", $3); }
       | factor DIV primary { $$ = new Binary($1, "/", $3); }
       | NOT factor         { $$ = new Unary("!", $2); }
       | primary            { $$ = $1; }
       ;

primary: TOK_NUM           { $$ = new Number($1); }
       | TOK_BOOL          { $$ = new BoolLit($1); }
       | TOK_ID LPAREN arg_list RPAREN { auto c = new Call($1); for(auto e:*$3) c->add(e); delete $3; $$ = c; }
       | TOK_ID            { $$ = new Identifier($1); }
       | LPAREN expr RPAREN{ $$ = $2; }
       ;

arg_list : /* empty */ { $$ = new vector<Expr*>(); }
         | nonempty_args { $$ = $1; }
         ;

nonempty_args : expr { $$ = new vector<Expr*>(); $$->push_back($1); }
