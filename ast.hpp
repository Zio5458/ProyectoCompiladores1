...
struct Call : Expr {
    ...
    explicit Call(string c) : callee(move(c)) {}
...
struct Block : Stmt {
    ...
    void add(Stmt* s) { stmts.emplace_back(s); }
    void print(ostream& os, int indent = 0) const override {
        ind(os, indent);
        os << "Block\n";
...
struct Param : Node {
    string name, type;
    Param(string n, string t) : name(move(n)), type(move(t)) {}
    void print(ostream& os, int indent = 0) const override {
        ind(os, indent);
...
struct Func : Node {
    ...
    explicit Func(string n) : name(move(n)) {}
    void addParam(Param* p) { params.emplace_back(p); }
    void setRet(string r) { retType = move(r); }
    void setBody(Block* b) { body.reset(b); }
    void print(ostream& os, int indent = 0) const override {
        ind(os, indent);
        os << "Func(\"" << name << "\" -> " << (retType.empty() ? "unit" : retType) << ")\n";
        ind(os, indent + 2); os << "Params\n";
        for (auto& p : params) p->print(os, indent + 4);
...
struct Program : Node {
    ...
    void add(Func* f) { funcs.emplace_back(f); }
    void print(ostream& os, int indent = 0) const override {
        ind(os, indent);
        os << "Program\n";
        for (auto& f : funcs) f->print(os, indent + 2);
    }
};